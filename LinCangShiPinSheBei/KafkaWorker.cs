...
public class KafkaWorker
{
    ...
    public static void sendDeviceMessage(string message,string type)
    {
        if (configDevice == null) { configDevice = new ProducerConfig { BootstrapServers = brokerList }; }
        if (type == "状态")
        {
            FileWorker.LogHelper.WriteLog("正在向kafka发送设备状态消息" + message);
        }
        else
        {
            FileWorker.LogHelper.WriteLog("正在向kafka发送设备基本消息" + message);
        }
        try
        {
        ...
        {
            FileWorker.LogHelper.WriteLog("发送设备信息失败:" + e.Message);
        }
    }
