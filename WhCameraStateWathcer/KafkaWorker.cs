...
public class KafkaWorker
{
    ...
    static string swipeTopicName = ConfigWorker.GetConfigValue("topicSwipe");
    static string personTopicName = ConfigWorker.GetConfigValue("topicPerson");
    ...
    static IProducer<Null, string> producerPerson = null;
    ...
    static ProducerConfig configPerson = null;
    ...
    public static void sendPersonMessage(string message)
    {
        if (configPerson == null) { configPerson = new ProducerConfig { BootstrapServers = brokerList }; }
        FileWorker.LogHelper.WriteLog("正在向kafka发送person消息" + message);
        try
        {
            if (producerPerson == null)
            {
                producerPerson = new ProducerBuilder<Null, string>(configPerson).Build();
            }
            producerPerson.Produce(personTopicName, new Message<Null, string> { Value = message }, handler);
            producerPerson.Flush(TimeSpan.FromSeconds(5));
        }
        catch (Exception e)
        {
            FileWorker.LogHelper.WriteLog("person error  " + e.Message);
        }
    }
    ...
    public static void startGetMessage()
    {
        ...
        using (consumerCommand = new ConsumerBuilder<Ignore, string>(configCommand).Build())
        {
            ...
            try
            {
                FileWorker.LogHelper.WriteLog("开始接收kafka消息");
                while (true)
                {
                    try
                    {
